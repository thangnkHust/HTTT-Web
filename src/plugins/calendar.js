...
var CHI = new Array("Tý", "S\u1EEDu", "D\u1EA7n", "M\\343o", "Th\\354n", "T\u1EF5", "Ng\u1ECD", "M\\371i", "Th\\342n", "D\u1EADu", "Tu\u1EA5t", "H\u1EE3i");
...
var	currentMonth =  today.main.getMonth() + 1;
var	currentYear = today.main.getFullYear();
...
/* Compute the sun segment at start (00:00) of the day with the given this.integral Julian day number.
 * The time zone if the time difference between local time and UTC: 7.0 for UTC+7:00.
 * The 	returns a number between 0 and 23.
 ...
 */
	...
	// DAYNAMES = new Array("CN", "T2", "T3", "T4", "T5", "T6", "T7"),
	// PRINT_OPTS = new OutputOptions(),
	// FONT_SIZES = new Array("9pt", "13pt", "17pt"),
	// TAB_WIDTHS = new Array("180px", "420px", "600px"),
	...
	printSelectedMonth() {
		...
		return this.printMonth(currentMonth, currentYear);
	},
	...
	printSelectedYear() {
		this.getSelectedMonth();
		return this.printYear(currentYear);
	},
	...
	printTable(mm, yy) {
		var i, j, k, solar;
		// lunar, cellClass, solarClass, lunarClass;
	...
	printHead(mm, yy) {
		...
		res += ('<tr><td colspan="2" class="navi-l">' + this.getPrevYearLink(mm, yy) + ' &nbsp;' + this.getPrevMonthLink(mm, yy) + '</td>\n');
	...
	printCell(lunarDate, solarDate, solarMonth, solarYear) {
		...
		if (lunarDate != null) res += (' title="' + this.getDayName(lunarDate) + '" onClick="alertDayInfo(' + args + ');"');
...
// }
	...
	alertDayInfo(dd, mm, yy, leap, jd, sday, smonth, syear) {
		var lunar = new this.LunarDate(dd, mm, yy, leap, jd);
		var s = this.getDayString(lunar, sday, smonth, syear);
		s += " \u00E2m l\u1ECBch\n";
		s += this.getDayName(lunar);
		s += "\nGi\u1EDD \u0111\u1EA7u ng\u00E0y: " + this.getCanHour0(jd) + " " + CHI[0];
		s += "\nTi\u1EBFt: " + TIETKHI[this.getSunLongitude(jd + 1, 7.0)];
	...
	showVietCal() {
		window.status = this.getCurrentTime() + " -+- " + this.getTodayString();
